...
async def update_chicken_death(chicken_death_id: int, chicken_death: ChickenDeathUpdate, session: T_Session):
    db_chicken_death = await session.scalar(select(ChickenDeathModel).where(ChickenDeathModel.id==chicken_death_id))

    if not db_chicken_death:
        raise HTTPException(status_code=HTTPStatus.NOT_FOUND, detail='Chicken death not found.')
    
    db_flock = await session.scalar(select(FlockModel.id).where(FlockModel.id==chicken_death.flock_id))

    if not db_flock:
        raise HTTPException(status_code=HTTPStatus.NOT_FOUND, detail='Flock not found.')
    
    db_death_reason = await session.scalar(select(ChickenDeathReasonModel.id).where(ChickenDeathReasonModel.id==chicken_death.reason_id))

    if not db_death_reason:
        raise HTTPException(status_code=HTTPStatus.NOT_FOUND, detail='Death reason not found.')
